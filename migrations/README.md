...
O arquivo `20260609_web_saas_baseline.sqlite.sql` é um snapshot auditável do schema SQLite inicial.

O arquivo `20260609_web_saas_baseline.postgres.sql` é a baseline inicial planejada para PostgreSQL gerenciado.

As migrações incrementais atuais adicionam sessões persistidas no banco, auditoria de login, solicitações de checkout de assinatura, registro idempotente de webhooks de cobrança e leads do site de divulgação.
